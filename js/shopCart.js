		...
		function cartDetails($btn){
			// getting cart id
			let id = $btn.parents('.box').attr('id');

			// getting image path
			let image = $btn.parents('.box').find('img').attr('src');

			// getting name of the product
			let name = $btn.parents('.box').find('#itemName').text();

			// getting price of the item
			let price = $btn.parents('.box').find('#itemPrice').text();
			let newPrice = price.split('$');

			return [id,'plus',image,name,newPrice[1]];                                 
		}
